...
def convert_coco_json2(json_dir):
    save_dir = make_dirs()  # output directory
    trash10 = trash10_class()
    ...
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f"Annotations {json_file}"):
            ...
            for ann in anns:
                ...
                cls = trash10[ann["category_id"]] # class
                box = [cls] + box.tolist()
                if box not in bboxes:
                    bboxes.append(box)
                # Segments
            ...
            with open((fn / f).with_suffix(".txt"), "a") as file:
                for i in range(len(bboxes)):
                    line = (*(segments[i] if False else bboxes[i]),)  # cls, box or segments
                    file.write(("%g " * len(line)).rstrip() % line + "\n")
...
if __name__ == "__main__":
    ...
    if source == "COCO":
        convert_coco_json(
            "/data/ephemeral/home/dataset/",  # directory with *.json
            use_segments=True,
            cls91to80=True,
        )
    
    elif source == "custom_COCO":
        convert_coco_json2(
