...
def main():
    annotation = '/data/ephemeral/dataset/train_rgt.json'
    ...
    for ann in data['annotations']:
        var_dict[ann['image_id']] = var_dict.get(ann['image_id'], 0) + 1
    ...
    for fold, (train_idx, val_idx) in enumerate(cv.split(X, y, groups), start=1):
        ...
        train_path = os.path.join(save_dir, f'train_rgt_{SEED}_fold_{fold}.json')
        with open(train_path, 'w') as f:
            json.dump(train_data, f, indent=4)
            
        val_path = os.path.join(save_dir, f'val_rgt_{SEED}_fold_{fold}.json')
